...
WHERE 
	dich_vu.ma_loai_dich_vu NOT IN (SELECT 
	hop_dong.ma_dich_vu 
FROM
	hop_dong
WHERE 
    ...
    ORDER BY dich_vu.ma_dich_vu ASC;
...
SELECT ma_nhan_vien AS'#ID' ,ho_va_ten, email, so_dien_thoai, ngay_sinh, dia_chi , "1" AS `TABLE`
FROM nhan_vien
UNION ALL
SELECT ma_khach_hang AS'#ID',ho_va_ten, email, so_dien_thoai, ngay_sinh, dia_chi, "2" AS `TABLE`
FROM khach_hang;
